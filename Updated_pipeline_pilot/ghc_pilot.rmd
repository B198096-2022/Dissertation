...
outdir <- "/Volumes/cmvm/scs/groups/pramacha-GROUP/Max_Hammer/QC/"
...
#Run SCTransform
for (i in range){
  SCTransform(Sce[[i]], vars.to.regress = "percent.mt", verbose = FALSE)
}
...
for (i in ncol(object_merged)){
  experiment_label <- c(experiment_group, sample_label)
}
...
plt <- DimPlot(object_merged, reduction = "umap.unintegrated", group.by = c("doublets"))
ggsave(filename = paste0(outdir, fig_name), plot = plt)
plt
