# DoubletFinder to run locally AND on Eddie

#When running on eddie begin session with logging onto interactive node
#qlogin -l h_vmem=8g

#Then I activated my conda environment 
#DoubletFinder requires seurat v4 or v3 objects 
...
#Alternatively, you can use different R package library locations 
#If using a conda enviornment then the chunk below can be ignored 
############################################################################
...
#Number of data sets 
range <- 1:<total data sets>
sample_author <- "<author>"
sample_name <- "<author>_<healthy/fibrosis>_"
sample_label <- "<sample_label>"
fig_lab <- sample_label

datadir <- paste0("/path/to/data/directory/")
outdir <- "/path/to/output/directory/"

#Desired filtering thresholds
mt_cutoff <- 30
nfeature_cutoff <- 100
ncount_cutoff <- 0
...
# calculate expected doublet number 
exp_d <- list()
for (i in range){
